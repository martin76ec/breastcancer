function r = nnLearning2(X,y,n_classes,hidden_layer_size,lambda,maxIter)
...
% W = 0.03*ones(L_out,1+L_in);
epsilon = 0.12;
h = 2*epsilon/(L_out*(1+L_in)-1);
W = -epsilon:h:epsilon;
W = reshape(W,L_out,1+L_in);%NOTE: Actually,this is NOT random initialisation!
...
function r = lab2bin(y,n_classes)
%************************************************by Holger Ortega
% y [mx1] - vector of labels. Labels must be integers from 1 to n_classes
% r - binary matrix
m = length(y);
possible_labels = 1:n_classes;
binLabels = zeros(m,n_classes);
diff = repmat(y,1,n_classes)-repmat(possible_labels,m,1);
indexes = diff==0;
binLabels(indexes)=1;
r = binLabels;